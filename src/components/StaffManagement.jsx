...
import { UserPlus, X, Phone, Mail, FileText, TrendingUp, DollarSign, Loader2, AlertCircle, Edit, Trash2, ChevronLeft, ChevronRight, Users } from 'lucide-react'
...
function StaffCard({ staff, onClick, isSelected }) {
  const roleColors = {
  ...
  return (
    <div
      onClick={onClick}
      className={`glass-card rounded-card p-5 card-hover cursor-pointer animate-slide-up ${isSelected ? 'border-2 border-primary-blue/60 shadow-[0_0_24px_rgba(74,144,226,0.35)]' : 'border border-transparent'}`}
    >
...
function StaffDrawer({ staff, onClose, onEdit, onDelete }) {
  ...
  return (
    <>
      <div
        ...
        onClick={onClose}
      />
      <div className="fixed top-0 right-0 h-full w-full sm:w-[420px] glass-card rounded-l-large z-50 overflow-y-auto animate-slide-in-right shadow-2xl">
        <div className="sticky top-0 bg-white/80 backdrop-blur-glass p-5 border-b border-gray-100 flex items-center justify-between">
        ...
        <div className="p-5 space-y-6">
          ...
          <div className="glass-card-inner rounded-xl p-4">
            ...
            <div className="space-y-3">
              <div className="flex justify-between p-3 rounded-lg bg-white/50">
                ...
                <span className="text-sm font-medium text-gray-800">{staff.hireDate ? new Date(staff.hireDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : 'N/A'}</span>
              </div>
              <div className="flex justify-between p-3 rounded-lg bg-white/50">
          ...
          <div className="glass-card-inner rounded-xl p-4">
            ...
            <div className="flex items-center justify-between p-3 rounded-lg bg-white/50">
              ...
              <span className={`text-sm font-medium px-3 py-1 rounded-full ${payrollColors[staff.payrollStatus] || 'bg-gray-100 text-gray-700'}`}>
                {staff.payrollStatus}
              </span>
            </div>
          </div>
          ...
          <div className="glass-card-inner rounded-xl p-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 rounded-lg bg-white/50">
                ...
                <span className={`text-sm font-medium px-3 py-1 rounded-full ${performanceColors[staff.performance] || 'bg-gray-100 text-gray-700'}`}>
                  {staff.performance}
                </span>
              </div>
...
export default function StaffScreen() {
  ...
  const classOptionsForFilter = [
    ...
    ...classOptions.map((cls) => ({ value: cls.id, label: cls.name })),
  ]
  ...
  const filteredStaff = staffList.filter((staff) => {
    ...
    const matchesClass = classFilter === 'all' || staff.assignedClassId === classFilter
      ...
      <div className="space-y-6">
        <div className="flex flex-col lg:flex-row gap-4 lg:items-center lg:justify-between">
          ...
          <PermissionButton
            ...
            disabled={isLoading}
          >
            {isLoading ? <LoadingSpinner /> : <>{<UserPlus size={18} />} Add Staff</>}
          </PermissionButton>
        </div>
                    ...
                    {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-3 py-1 rounded text-sm ${currentPage === pageNum ? 'bg-primary-blue text-white' : 'hover:bg-gray-100'}`}
                        >
                          {pageNum}
                        </button>
                      )
